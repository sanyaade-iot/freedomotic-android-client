...
public class Payload implements Serializable {
   ...
   public void enqueueStatement(Statement s) {
       if (s != null) {
          ...
          // freedomotic.logger.warning("Attempt to add a null or empty statement in payload. Statement discarded");
       }
   }
...
//                           freedomotic.logger.info("[NOT CONSISTENT] The trigger has fail the check because the event statement '" + event.toString() +
//                                   "' is not contistent with trigger statement '" + trigger.attribute + " " + trigger.operand + " " + trigger.value + "'");
           ...
           for (int i = 0; i < results.size(); i++) {
               if (getStatements().get(i).getLogical().equalsIgnoreCase("AND")) {
                   ...
                   //freedomotic.logger.severe("AND: " + consistent + " " + results.get(i) + " consistent is: " + consistent);
               } else {
                   if (getStatements().get(i).getLogical().equalsIgnoreCase("OR")) {
                      // freedomotic.logger.warning("This payload uses the experimental feature 'OR'. Use it carefully because is still not tested properly");
                       consistent = consistent || results.get(i);
                       //freedomotic.logger.severe("OR: " + consistent + " " + results.get(i) + " consistent is: " + consistent);
                   }
               }
           }
       }
   ...
   private static boolean isStatementConsistent(String triggerOperand, String triggerValue, String eventValue) {
       ...
       if (triggerOperand.equals(Statement.GREATER_THEN)) { //event operand="EQUALS", trigger operand="GREATER_THEN"
           ...
           //    freedomotic.logger.warning(Statement.GREATER_THEN.toString() + " operator can be applied only to integer values");
               return false;
           }
       ...
       if (triggerOperand.equals(Statement.LESS_THEN)) { //event operand="EQUALS", trigger operand="LESS_THEN"
           try {
               ...
               //is not a number
             //  freedomotic.logger.warning(Statement.LESS_THEN.toString() + " operator can be applied only to integer values");
               return false;
           }
       }
               //applies only to integer values
       if (triggerOperand.equals(Statement.GREATER_EQUAL_THEN)) { //event operand="EQUALS", trigger operand="GREATER_THEN"
           try {
            ...
            //   freedomotic.logger.warning(Statement.GREATER_EQUAL_THEN.toString() + " operator can be applied only to integer values");
               return false;
           }
       ...
       if (triggerOperand.equals(Statement.LESS_EQUAL_THEN)) { //event operand="EQUALS", trigger operand="LESS_THEN"
           try {
               ...
               //is not a number
            //   freedomotic.logger.warning(Statement.LESS_EQUAL_THEN.toString() + " operator can be applied only to integer values");
               return false;
           }
       }
       return false;
   ...
   public Statement findAttribute(String key) {
       ...
       while (it.hasNext()) {
           ...
           //freedomotic.logger.info("DEBUG: contains attribute " + s.getAttribute());
