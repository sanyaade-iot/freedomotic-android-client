...
public class Payload implements Serializable {
                ...
                if (event != null) {
                    ...
                    if (trigger.logical.equalsIgnoreCase("SET")) {
                        ...
                        if (!isStatementConsistent(trigger.operand, trigger.value, event.value)) {
                            //Freedom.logger.info("       The trigger has fail the check because the event value '" + event.value + "' is not " + trigger.operand + " the value '" + trigger.value + "' found in trigger for attribute '" + trigger.attribute + "'");
                            return false;
                        } else {
                            //Freedom.logger.info("DEBUG TRIGGER TRUE: " + trigger.value + " " + trigger.operand + " " + event.value);
                        }
                    }
                }
            }
            return true; //all statements consistent
        } else {
            return false;
        }
    }
    ...
    private static boolean isStatementConsistent(String triggerOperand, String triggerValue, String eventValue) {
        if (triggerOperand.equals(Statement.EQUALS)) { //event operand="EQUALS", trigger operand="EQUALS"
...
//        while (it.hasNext()) {
//            Statement s = (Statement) it.next();
//            buffer.append("<li>" + s.toString() + "</li>\n");
//        }
//        buffer.append("</ul>\n");
//        return buffer.toString();
//    }
